...
const Item = ({address, attributes, description, images, title, user, published, star, id}) => {
    ...
    const handleStart = (star) => {
        ...
        for (let i = 1; i <= star; i++)
            stars.push(<IoIosStar color='#FEBB00' className='star-item' />)
        return stars
    }
    ...
    return (
        <div className='flex w-full border-red-500 border-t py-[6px] mt-4'>
            ...
            <Link to={`chi-tiet/${title}/${id}`} className='w-2/5 flex flex-wrap gap-[2px] items-center mr-1 relative cursor-pointer'>
                {
                    images.length > 0 && images.filter((i, index) => indexs.some(i => i === index))?.map((i, index) => {
                        return (
                            <img key={index} src={i} alt='preview' className='w-[110px] h-[130px] object-cover'/>
                        )
                    })
                }
                ...
                <span onMouseEnter={() => setIsHoverHeart(true)}
            ...
            <div className='w-3/5'> 
                ...
                <p className='text-gray-500 text-sm mt-2 w-full h-[65px] text-ellipsis overflow-hidden'>
                    {description}
                </p>
                ...
                <div className='flex items-center mt-2 justify-between'>
                    <div className='flex items-center gap-1'>
                        ...
                        alt='avatar' className='w-[30px] h-[30px] object-cover rounded-full' />
